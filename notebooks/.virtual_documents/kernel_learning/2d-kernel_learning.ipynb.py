...
assert not std_normalize
learner = models.KernelLearner(key,
                                target,
                                sizes,
...
assert not sample_every
samples = proposal.sample(400, subkey)
learner.train(samples, n_steps=500, progress_bar=True)


ksds_post = get_ksds(learner.get_kernel(learner.get_params()))

inducing_samples = proposal.sample(500)
learned_stein_gradient = learner.get_phistar(samples=inducing_samples)
def kl_gradient(x):
...
def rescale_kl_gradient(key, kl_grad: callable):
    """return rescaled kl_grad (rescale to fit learned stein gradient)"""
