...
test = random.normal(subkey, (3, 3)) * 5
test


utils.remove_diagonal(test)
...
def compare(dist):
    ...
    true_f = grad(lambda x: target_logp(x) - dist.logpdf(x))
    sd = stein.stein_discrepancy(samples, target_logp, true_f)
    true_sd = true_stein_discrepancy(samples, dist.logpdf, true_f)
    l2 = utils.l2_norm_squared(samples, true_f)
    print("SD:", sd)
    print("True SD:", true_sd)
    print("L2 norm:", l2)
    return sd, true_sd, l2
...
t = hk.transform(lambda *args: Test()(*args))


x = np.array([1,2,3.])
params = t.init(subkey, x, lambda x: x)


f = lambda x: 3*x
t.apply(params, subkey, x, f)


schedule_fn = optax.polynomial_schedule(
    init_value=1., end_value=0.1, power=.2, transition_steps=100)
plt.plot([schedule_fn(step) for step in range(130)])
plt.ylim((0, 1.1))
